...
async def addbalance_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle /addbalance command – admin adds credits to a user"""
    ...
    if not context.args or len(context.args) < 2:
        await update.message.reply_text(
            "Usage: /addbalance <user_id> <credit_amount>\n\n"
...
async def white_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    ...
    try:
        ...
        if db.unblock_user(target_user_id):
            await update.message.reply_text(
                f"✅ User {target_user_id} has been removed from the blacklist."
            )
        else:
            await update.message.reply_text("Operation failed. Please try again later.")
    except ValueError:
...
async def genkey_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle /genkey command – admin generates license keys"""
    if await reject_group_command(update):
        return
    ...
    if not context.args or len(context.args) < 2:
        await update.message.reply_text(
            "Usage: /genkey <key> <credits> [max_uses] [expire_days]\n\n"
            "Examples:\n"
            "/genkey wandouyu 20 - Generate a 20-credit key (single-use, never expires)\n"
            "/genkey vip100 50 10 - Generate a 50-credit key (10 uses, never expires)\n"
    ...
    try:
        ...
        if db.create_card_key(key_code, balance, user_id, max_uses, expire_days):
            ...
            await update.message.reply_text(
                "The key already exists or generation failed. Please choose a different key."
            )
    except ValueError:
        await update.message.reply_text("Invalid parameter format. Please enter valid numbers.")
...
async def listkeys_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle /listkeys command – admin views license key list"""
    if await reject_group_command(update):
        return
    ...
    if not keys:
        await update.message.reply_text("No license keys available.")
        return

    msg = "📋 License Key List:\n\n"
    ...
    if len(keys) > 20:
        msg += f"\n(Showing first 20 only, total {len(keys)})"
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    ...
    for uid in user_ids:
        try:
            ...
            success += 1
            await asyncio.sleep(0.05)  # Rate limit to avoid triggering restrictions
