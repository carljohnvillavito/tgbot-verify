...
def get_about_message() -> str:
    ...
    return (
        ...
        "\n"
        "How to earn credits:\n"
        "- 1 credit for registration\n"
        "- Daily check-in: +1 credit\n"
        "- Invite friends: +2 credits per person\n"
        "- Use license keys (according to key rules)\n"
        f"- Join the channel: {CHANNEL_URL}\n"
        "\n"
        "How to use:\n"
...
def get_help_message(is_admin: bool = False) -> str:
    ...
    msg = (
        ...
        "/invite - Generate an invite link (+2 credits per person)\n"
        "/use <key> - Redeem credits using a license key\n"
    ...
    if is_admin:
        msg += (
            ...
            "/genkey <key> <credits> [uses] [days] - Generate a license key\n"
            "/listkeys - View license key list\n"
...
def get_insufficient_balance_message(current_balance: int) -> str:
    ...
    return (
        ...
        "- Use a license key /use <key>"
    )


def get_verify_usage_message(command: str, service_name: str) -> str:
    """Get verification command usage instructions"""
    return (
